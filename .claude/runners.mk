...
# Usage: make setup-all ANTHROPIC_API_KEY=sk-ant-...
...
.PHONY: help setup-all status start stop restart remove list migrate

help:
	@echo "GitHub Actions Runner Management"
	@echo ""
	@echo "Usage:"
	@echo "  make setup-all ANTHROPIC_API_KEY=sk-ant-...  # Setup all runners from config"
	@echo "  make setup REPO=owner/repo                   # Setup single runner"
	...
	@echo "  make migrate                                 # Migrate old runners to new location"
	@echo ""
...
setup-all:
	...
	@for repo in $(REPOS); do \
		...
		$(SCRIPT_DIR)/setup-runner.sh "$$repo" "$(ANTHROPIC_API_KEY)" || true; \
	done
...
setup:
	...
	$(SCRIPT_DIR)/setup-runner.sh "$(REPO)" "$(ANTHROPIC_API_KEY)"
...
# Migrate runners from old locations (~/actions-runner-*) to new location
migrate:
